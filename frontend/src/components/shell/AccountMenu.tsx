...
function MenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className="flex h-8 w-full items-center gap-2.5 rounded-md px-2.5 text-[12px] text-ink-dim
      ...
      >
        <span
          className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full
                     border border-border bg-surface-3 text-[11px] font-medium text-ink-dim"
        >
          {account.initials}
        </span>
        <span
        ...
        >
          <span className="block truncate text-[12px] font-medium text-ink">{account.name}</span>
